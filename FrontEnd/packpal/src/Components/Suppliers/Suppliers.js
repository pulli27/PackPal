...
export default function Employee() {
  ...
  const openView = (emp) => {
    ...
    else dlg.setAttribute("open", ""); // basic fallback if <dialog> API missing
  };
  ...
  const downloadCSV = () => {
    ...
    // Proper RFC4180-ish quoting: wrap each field in quotes and escape any quotes inside
    const toCSV = (rows) =>
      rows
        .map((r) =>
    ...
    <div>
      ...
      <dialog ref={modalRef} className="modal" onClose={closeView}>
        <div className="modal-card">
          ...
          <p className="muted">
            {selected
              ? `Age ${selected.age} • Service ${selected.service} years`
              : ""}
          </p>
          <div className="skill-wrap">
