...
function Clutches() {
  ...
  // - If you place files in public/images, use `img("c2.jpg")`
  // - If your product entry has "images/c2.jpg" already, this will still work.
  const img = (nameOrPath) => {
    if (!nameOrPath) return "";
    // If it already looks like a path with "images/", keep it as is,
    // otherwise assume it's a filename inside public/images
  ...
  const [cart, setCart] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("pacpal_cart") || "[]");
    } catch {
      return [];
    }
  });
        ...
        >
          <div className="modal-card">
            <div className="modal-img">
              <img id="mImg" src={img(modalProduct.img)} alt={modalProduct.title} />
            </div>
