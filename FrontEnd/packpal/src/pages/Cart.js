...
// optional: endpoint to bump product counters (will not block checkout if it fails)
...
export default function Cart() {
  ...
  useEffect(() => {
    setErrors({
      ...
      ...(showPanel ? validateCard(form) : {}), // card errors only after panel shown
    });
  }, [form, showPanel]);
  ...
  const processPayment = async () => {
    ...
    try {
      // 1) (optional) bump product side-effects
      await Promise.all(
        cart.map((it) =>
          axios
            .post(
      ...
      // 2) create one transaction per cart line
      await Promise.all(
        cart.map((it) => {
          ...
          const payload = {
            ...
            customerId: "",
            fmc: false,
            // IMPORTANT: Your backend expects Mongo ObjectId in productId
      ...
      setErr(
        "Payment failed: " + (e?.response?.data?.message || e?.response?.data?.error || e.message || "Unknown error")
      );
    } finally {
      setProcessing(false);
    }
            ...
            <div className="payment-form">
              ...
              <div className="form-section">
                ...
                <div className="form-row">
                  <div
                    className={`form-group ${invalid("firstName") ? "has-error" : ""}`}
                  >
                  ...
                  <div
                    className={`form-group ${invalid("lastName") ? "has-error" : ""}`}
                  >
                ...
                <div className="form-row">
                  ...
                  <div className={`form-group ${invalid("zip") ? "has-error" : ""}`}>
                    ...
                    />
                    {invalid("zip") && (
                      <div className="field-error">{errors.zip}</div>
                    )}
                  </div>
                </div>
              </div>
              ...
              {showPanel && (
                <div className="panel-blue" ref={panelRef}>
                  ...
                  <div>
                    <div
                      className={`form-group ${invalid("cardNumber") ? "has-error" : ""}`}
                    >
                    ...
                    <div className="form-row">
                      ...
                      <div
                        className={`form-group ${invalid("cardName") ? "has-error" : ""}`}
                      >
        ...
        {step === "success" && (
          <section className="page active">
            <div className="success-message">
              ...
              >
                Thank you for choosing PackPal! Your smart bag order has been
                recorded.
              </p>
              <div style={{ textAlign: "center", marginTop: 20 }}>
